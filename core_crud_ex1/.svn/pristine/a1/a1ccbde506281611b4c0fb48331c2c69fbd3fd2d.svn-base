...
function inputEmp(iuser_id_nm, iuser_nm){
	$('#iuser_id_nm').val(iuser_id_nm);
	$('#iuser_nm').val(iuser_nm);
}

function empClick(iuser_id_nm, iuser_nm) {
	window.opener.inputEmp(iuser_id_nm, iuser_nm);
	self.close();
}
			...
			success : function(data){
				...
				if(data.empList.length > 0){
					...
					for (var i = 0; i < data.empList.length; i++) {
						tbodyContent = "<tr class='schRstTblTr' onclick=\"empClick('"+data.empList[i].iuser_id_nm+"', '"+data.empList[i].iuser_nm+"');\">"
